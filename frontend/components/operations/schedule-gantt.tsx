...
import type { FactorySnapshot, MachineRiskReport, Schedule, ScheduleAssignment } from "@/types/generated/operations";

export interface ScheduleGanttProps {
  schedule: Schedule;
  snapshot: FactorySnapshot;
  riskReports?: readonly MachineRiskReport[];
  label: string;
...
function assignmentContext(assignment: ScheduleAssignment, snapshot: FactorySnapshot, risks: readonly MachineRiskReport[]) {
  ...
  const predecessors = operation?.predecessor_operation_ids ?? [];
  return {
    ...
    dependencies: predecessors.length ? predecessors.join(", ") : "None returned",
...
export function ScheduleGantt({ schedule, snapshot, riskReports = [], label, machineFilter = "", assignmentTypeFilter = "" }: ScheduleGanttProps) {
  ...
  const lane = (key: string, title: string, assignments: ScheduleAssignment[]) => (
    <div key={key} aria-label={`${title} lane`} className="grid min-h-14 grid-cols-[120px_1fr] border-t border-[var(--color-stone-border)]">
    ...
    <div className="p-4 sm:p-5">
      ...
      <div className="mt-4 overflow-x-auto">
        <table className="w-full min-w-[820px] text-left text-xs">
          ...
          <tbody className="divide-y divide-[var(--color-stone-border)]">{included.map((assignment) => { const context = assignmentContext(assignment, snapshot, riskReports); const technicianIds = assignment.technician_ids ?? []; return <tr key={assignment.assignment_id}><td className="py-2 font-mono font-semibold">{context.task}</td><td className="py-2">{assignment.machine_id}{technicianIds.length ? ` · ${technicianIds.join(", ")}` : ""}</td><td className="py-2">{formatTime(assignment.start_at)}–{formatTime(assignment.end_at)}</td><td className="py-2"><OperationsStatus value={assignment.status} /></td><td className="py-2">{context.risk} / {context.priority}</td><td className="py-2">{context.dependencies}</td></tr>; })}</tbody>
        </table>
      </div>
