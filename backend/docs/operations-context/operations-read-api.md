...
- Persist the normalized payload and hash in the same transaction. Reads recompute hashes, revalidate canonical schema and compare IDs/schema/timestamps and current schedule content against its own basis snapshot.current_schedule. Invalid version/hash fails closed.
- Composite keys preserve canonical string IDs and schedule revision: snapshot `(factory_id,snapshot_id)`, schedule `(factory_id,schedule_id,revision)`. Composite FKs fence factory and snapshot relationships for current head. Schedule FK preserves factory scope independently of head.snapshotId; schedule.snapshotId retains its immutable basis. See [snapshot ingestion](snapshot-ingestion.md) for migration impact and concurrency.
