...
Successful output identifies `factory-demo-01`, `snapshot-demo-20260922-0800`, six machines and hashes. Replaying unchanged seed succeeds without modifying timestamps/head. Conflicting stable IDs/content or a different current head return OPERATIONS_SEED_CONFLICT and roll back; seed never implicitly resets or overwrites. Concurrent same-factory seed runs serialize via a transaction-scoped advisory lock, shared by the separate demo reset command. New live head writers are outside this PR and must use their own coordinated revision checks.
...
- Composite keys preserve canonical string IDs and schedule revision: snapshot `(factory_id,snapshot_id)`, schedule `(factory_id,schedule_id,revision)`. The head fences snapshot and schedule by factory, while the schedule retains its own immutable basis snapshot. A fresh observation snapshot can therefore become current without republishing the existing schedule.
