...
EpisodeSummary is stored in episode_summaries before COMPLETED is written, in the same transaction. Missing/invalid summaries, engine failures and persistence failures leave the episode FAILED with previously committed steps intact. Diagnostics are stored on the episode and in AuditLog when the worker still owns its lease.

Heartbeats renew the lease independently of event delivery. Expired RUNNING episodes are marked FAILED during polling; partial data is retained. They are never automatically replayed: the adapter has no resume/checkpoint contract yet. Do not manually reset partially executed episodes to PENDING; create a new episode/experiment instead. A stale owner cannot write after lease expiry or after another token owns the episode. Multiple workers may poll the same database safely. When a process or DB connection fails, stale recovery records WORKER_LEASE_EXPIRED once the database is available.
...
Fake execution requires both NODE_ENV=development (or test) and ENGINE_RUNNER=fake. Without an explicit runner selection startup fails. FakeEngineRunner produces deterministic fixtures marked fixture: true; it is not a scientific simulation. Copy the worker settings from .env.example into the local environment when needed.
...
The request contains episodeId, environmentConfig, policyId, policyVersion and seed. Emit contiguous steps starting at zero, with one ordered observation/recommendation/result per step, followed by exactly one EpisodeSummary and stream end. No events are allowed after a terminal StepResult except the summary. The current v2 payload contract is in src/engine/contracts.js; real adapters use fixture: false. Contract/domain changes for the incoming engine should be agreed separately.
